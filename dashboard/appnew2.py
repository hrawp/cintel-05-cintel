...
# --------------------------------------------
# Define the UI with a sidebar layout
app_ui = ui.page_fluid(
    ui.layout_sidebar(
        # Sidebar section where the slider is placed
        ui.sidebar_panel(
            ui.input_slider("dayornight", "Choose Day or Night Temperatures", min=0, max=1, value=1)
        ),
        # Main content area
        ui.main_panel(
            ui.output_text_verbatim("temp_output")
        )
    )
)
...
def server(input, output, session):
    ...
    # Reactive calculation to process the slider value
    @reactive.Calc
    def filtered_data():
        # Access the slider value reactively
        daynight = input.dayornight()

        # Conditional logic based on the slider value
        if daynight == 1:
            return "Night temperatures selected"
        else:
            return "Day temperatures selected"
    
    # Output the result of the reactive calculation to the UI
    output.temp_output = filtered_data

# Run the app
shiny.app(ui=app_ui, server=server)
